...
classdef simpleGameEngine < handle
    ...
    methods
        ...
        function drawScene(obj, background_sprites, foreground_sprites)
            ...
            if (~isempty(obj.custom_sprites))
                for customSpriteIndex = 1:length(obj.custom_sprites)
                    currentSpriteData = obj.custom_sprites{customSpriteIndex};
                    
                    width = currentSpriteData.width;
                    xCoord = currentSpriteData.xCoord;
                    yCoord = currentSpriteData.yCoord;
                    spriteIndex = currentSpriteData.spriteIndex;
                    
                    spriteObject = obj.sprites{spriteIndex};
                    
                    spriteTransparency = obj.sprites_transparency{spriteIndex};

                        originalSize = size(spriteObject);
                        ...
                        scaledWidth = scaledSize(1);
                            ...
                            spriteData = scene_data(yCoord+1:yCoord+scaledWidth, xCoord+1:xCoord+scaledWidth,:);
                            
                        spriteData = spriteObject .* (spriteTransparency/255) + ...
                                spriteData .* ((255-spriteTransparency)/255);

                        scene_data(yCoord+1:yCoord+scaledWidth, xCoord+1:xCoord+scaledWidth,:) = spriteData;
        ...
        function queueSprite(obj, width, xCoord, yCoord, spriteIndex, id)
            nextIndex = length(obj.custom_sprites) + 1;
            
            obj.custom_sprites{nextIndex}.width = width;
            obj.custom_sprites{nextIndex}.xCoord = xCoord;
            obj.custom_sprites{nextIndex}.yCoord = yCoord;
            obj.custom_sprites{nextIndex}.spriteIndex = spriteIndex;
            obj.custom_sprites{nextIndex}.id = id;
        end
        
        function removeSprite(obj, removeId)
            if ~isempty(obj.custom_sprites)
                
                index = 1;
                
                while index < length(obj.custom_sprites)
                    id = obj.custom_sprites{index}.id;
                    if id == removeId
                        obj.custom_sprites(index) = [];
                    else
                        index = index + 1;
                    end
                end
